...
export function getSlackUsers(
	...
	while (departmentCounter < allDepartmentKeys.length) {
		...
		if (teamKeys.length === 0) {
			throw new Error(
				"No Team Group (Dev_Team_1, SomethingCool1, etc.) found in JSON file",
			);
		}
		...
		while (selectedTeamCounter < teamKeys.length) {
			...
			const selectedGroupSubTeam =
				selectedDepartment[teamKeys[selectedTeamCounter]];
...
export function getSlackUsersAlt(
	...
	throw new Error(
		`Slack user Id: ${slackUserId} could not be found in JSON file`,
	);
}
