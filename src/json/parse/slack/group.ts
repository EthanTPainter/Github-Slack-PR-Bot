 ...
 */
export function getSlackGroup(githubUser: string, json: any): SlackUser {
			...
			if (group === undefined) {
				...
				while (leadCounter < leadKeys.length) {
					...
					if (leadKeys[leadCounter] === githubUser) {
						logger.debug(
							`Found Github user ${githubUser}. Slack group: ${group}`,
						);
						return group;
					}
					leadCounter++;
			...
			if (group === undefined) {
				...
				while (leadCounter < leadKeys.length) {
					...
					if (selectedLead.Slack_Id === slackUserId) {
						logger.debug(
							`Found Slack user id ${slackUserId}. Slack group: ${group}`,
						);
						return group;
					}
					leadCounter++;
