...
export function getSlackLeads(
	...
	while (teamCounter < allTeamKeys.length) {
		...
		while (selectedTeamTypeCounter < teamGroupKeys.length) {
			const selectedGroupSubTeam =
				selectedTeamGroup[teamGroupKeys[selectedTeamTypeCounter]];
			if (selectedGroupSubTeam.Users === undefined) {
				throw new Error(
					`No Users defined for team: ${teamGroupKeys[selectedTeamTypeCounter]}`,
				);
			}
			...
			if (users.Leads === undefined) {
				throw new Error(
					`Leads not defined for team: ${teamGroupKeys[selectedTeamTypeCounter]}`,
				);
			}
			if (users.Members === undefined) {
				throw new Error(
					`Members not defined for team: ${teamGroupKeys[selectedTeamTypeCounter]}`,
				);
			}
			...
			while (leadCounter < leadKeys.length) {
				if (leadKeys[leadCounter] === githubUser) {
					logger.debug(
						`Found GitHub user ${githubUser}. Leads: ${Object.values(
							users.Leads,
						)}`,
					);
	...
	while (departmentCounter < allDepartmentKeys.length) {
		...
		while (selectedTeamCounter < teamKeys.length) {
			...
			if (selectedTeam.Users === undefined) {
				throw new Error(
					`No Users defined for team: ${teamKeys[selectedTeamCounter]}`,
				);
			}
			...
			if (users.Leads === undefined) {
				throw new Error(
					`Leads not defined for team: ${teamKeys[selectedTeamCounter]}`,
				);
			}
			if (users.Members === undefined) {
				throw new Error(
					`Members not defined for team: ${teamKeys[selectedTeamCounter]}`,
				);
			}
			...
			while (leadCounter < leadValues.length) {
				...
				if (selectedLead.Slack_Id === slackUser.Slack_Id) {
					logger.debug(
						`Found Slack user ${slackUser}. Leads: ${Object.values(
							users.Leads,
						)}`,
					);
