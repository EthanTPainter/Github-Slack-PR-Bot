...
import { SlackUser, TeamOptions, JSONConfig } from "../../../../models";
...
export function constructLeadCheck(
	...
	// Prepare beginning lead approval string
	let leadCheck: string;
	if (
		options.Num_Required_Lead_Approvals === 0 &&
		leadsApproving.length === 0 &&
		leadsReqChanges.length === 0
	) {
		return "";
	} else if (options.Num_Required_Lead_Approvals === 0) {
	...
	if (
		leadsApproving.length + leadsReqChanges.length >=
