...
import { updateClose } from "../../../../../src/dynamo/update";
...
describe("Dynamo.UpdateClose", () => {
  ...
  it("should update queues with a closed PR -- only PR in each queue & alert members before leads", async () => {
  ...
  it("should update queues with a closed PR -- other PRs in queues", async () => {
    ...
    const newPR = {
      ...
      title: "VALID PR TITLE #1",
      url: "www.github.com/ethantpainter",
      standard_members_alert: [slackMember1.Slack_Id,
        slackMember3.Slack_Id],
      members_approving: [],
      member_complete: false,
