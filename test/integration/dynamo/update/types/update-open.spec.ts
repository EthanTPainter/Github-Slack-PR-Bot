...
describe("Dynamo.UpdateOpen", () => {
	...
	it("should update a queue with an opened PR -- owned by a member & alert members before leads", async () => {
		...
		expect(teamQueue[0].events[0].action).deep.equal(
			expectedUpdatedQueue[0].events[0].action,
		);
		expect(teamQueue[0].events[0].user).deep.equal(
			expectedUpdatedQueue[0].events[0].user,
		);
		expect(teamQueue[0].lead_complete).deep.equal(
			expectedUpdatedQueue[0].lead_complete,
		);
		expect(teamQueue[0].standard_leads_alert).deep.equal(
			expectedUpdatedQueue[0].standard_leads_alert,
		);
		expect(teamQueue[0].leads_approving).deep.equal(
			expectedUpdatedQueue[0].leads_approving,
		);
		expect(teamQueue[0].member_complete).deep.equal(
			expectedUpdatedQueue[0].member_complete,
		);
		expect(teamQueue[0].standard_members_alert).deep.equal(
			expectedUpdatedQueue[0].standard_members_alert,
		);
		expect(teamQueue[0].members_approving).deep.equal(
			expectedUpdatedQueue[0].members_approving,
		);
		...
		expect(member2Queue[0].events[0].action).deep.equal(
			expectedUpdatedQueue[0].events[0].action,
		);
		expect(member2Queue[0].events[0].user).deep.equal(
			expectedUpdatedQueue[0].events[0].user,
		);
		expect(member2Queue[0].lead_complete).deep.equal(
			expectedUpdatedQueue[0].lead_complete,
		);
		expect(member2Queue[0].standard_leads_alert).deep.equal(
			expectedUpdatedQueue[0].standard_leads_alert,
		);
		expect(member2Queue[0].leads_approving).deep.equal(
			expectedUpdatedQueue[0].leads_approving,
		);
		expect(member2Queue[0].member_complete).deep.equal(
			expectedUpdatedQueue[0].member_complete,
		);
		...
		expect(member3Queue[0].events[0].action).deep.equal(
			expectedUpdatedQueue[0].events[0].action,
		);
		expect(member3Queue[0].events[0].user).deep.equal(
			expectedUpdatedQueue[0].events[0].user,
		);
		expect(member3Queue[0].lead_complete).deep.equal(
			expectedUpdatedQueue[0].lead_complete,
		);
		expect(member3Queue[0].standard_leads_alert).deep.equal(
			expectedUpdatedQueue[0].standard_leads_alert,
		);
		expect(member3Queue[0].leads_approving).deep.equal(
			expectedUpdatedQueue[0].leads_approving,
		);
		expect(member3Queue[0].member_complete).deep.equal(
			expectedUpdatedQueue[0].member_complete,
		);
		expect(member3Queue[0].standard_members_alert).deep.equal(
			expectedUpdatedQueue[0].standard_members_alert,
		);
		expect(member3Queue[0].members_approving).deep.equal(
			expectedUpdatedQueue[0].members_approving,
		);
	...
	it("should update a queue with an opened PR -- owned by a member & alert members & leads", async () => {
		...
		expect(lead1Queue[0].events[0].action).deep.equal(
			expectedUpdatedQueue[0].events[0].action,
		);
		expect(lead1Queue[0].events[0].user).deep.equal(
			expectedUpdatedQueue[0].events[0].user,
		);
		expect(lead1Queue[0].lead_complete).deep.equal(
			expectedUpdatedQueue[0].lead_complete,
		);
		expect(lead1Queue[0].standard_leads_alert).deep.equal(
			expectedUpdatedQueue[0].standard_leads_alert,
		);
		expect(lead1Queue[0].leads_approving).deep.equal(
			expectedUpdatedQueue[0].leads_approving,
		);
		expect(lead1Queue[0].member_complete).deep.equal(
			expectedUpdatedQueue[0].member_complete,
		);
		expect(lead1Queue[0].standard_members_alert).deep.equal(
			expectedUpdatedQueue[0].standard_members_alert,
		);
		expect(lead1Queue[0].members_approving).deep.equal(
			expectedUpdatedQueue[0].members_approving,
		);
		...
		expect(lead2Queue[0].events[0].action).deep.equal(
			expectedUpdatedQueue[0].events[0].action,
		);
		expect(lead2Queue[0].events[0].user).deep.equal(
			expectedUpdatedQueue[0].events[0].user,
		);
		expect(lead2Queue[0].lead_complete).deep.equal(
			expectedUpdatedQueue[0].lead_complete,
		);
		expect(lead2Queue[0].standard_leads_alert).deep.equal(
			expectedUpdatedQueue[0].standard_leads_alert,
		);
		expect(lead2Queue[0].leads_approving).deep.equal(
			expectedUpdatedQueue[0].leads_approving,
		);
		expect(lead2Queue[0].member_complete).deep.equal(
			expectedUpdatedQueue[0].member_complete,
		);
		expect(lead2Queue[0].standard_members_alert).deep.equal(
			expectedUpdatedQueue[0].standard_members_alert,
		);
		expect(lead2Queue[0].members_approving).deep.equal(
			expectedUpdatedQueue[0].members_approving,
		);
		...
		expect(lead3Queue[0].events[0].action).deep.equal(
			expectedUpdatedQueue[0].events[0].action,
		);
		expect(lead3Queue[0].events[0].user).deep.equal(
			expectedUpdatedQueue[0].events[0].user,
		);
		expect(lead3Queue[0].lead_complete).deep.equal(
			expectedUpdatedQueue[0].lead_complete,
		);
		expect(lead3Queue[0].standard_leads_alert).deep.equal(
			expectedUpdatedQueue[0].standard_leads_alert,
		);
		expect(lead3Queue[0].leads_approving).deep.equal(
			expectedUpdatedQueue[0].leads_approving,
		);
		expect(lead3Queue[0].member_complete).deep.equal(
			expectedUpdatedQueue[0].member_complete,
		);
		expect(lead3Queue[0].standard_members_alert).deep.equal(
			expectedUpdatedQueue[0].standard_members_alert,
		);
		expect(lead3Queue[0].members_approving).deep.equal(
			expectedUpdatedQueue[0].members_approving,
		);
