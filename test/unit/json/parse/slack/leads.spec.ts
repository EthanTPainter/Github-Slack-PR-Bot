...
describe("getSlackLeads", () => {
	...
	it("should get only slack lead name", () => {
		...
		const expected = [validJSON_1.Departments.Dep1.Team1.Users.Leads.Ethan];
	...
	it("should throw an error -- No Team found", () => {
		...
		expect(() => getSlackLeads(githubUser, invalidJSON)).to.throw(
			expected.message,
		);
	});
	...
	it("should throw an error -- No Users defined in JSON", () => {
		...
		expect(() => getSlackLeads(githubUser, invalidJSON as any)).to.throw(
			expected.message,
		);
	});
