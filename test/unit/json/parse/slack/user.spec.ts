...
describe("getSlackUser", () => {
	...
	it("should retrieve the slack username given a lead github user", () => {
		...
		const result = getSlackUser(githubUser, validJSON);
		const expected =
			validJSON.Departments.Developers.PhillyDevTeam.Users.Leads.andrew;
	...
	it("should throw an error -- github user not found", () => {
		...
		const expected = new Error(
			`GitHub user: ${githubUser} could not be found in JSON file`,
		);

		expect(() => getSlackUser(githubUser, validJSON)).to.throw(
			expected.message,
		);
	});
	...
	it("should throw an error -- No Teams", () => {
		...
		expect(() => getSlackUser(githubUser, invalidJSON)).to.throw(
			expected.message,
		);
	});
	...
	it("should throw an error -- No Users", () => {
		...
		expect(() => getSlackUser(githubUser, invalidJSON as any)).to.throw(
			expected.message,
		);
	});
...
describe("getSlackUserAlt", () => {
	...
	it("should throw an error -- Slack user id not found", () => {
		...
		const expected = new Error(
			`Slack user id: ${slackUserId} not found in JSON file`,
		);

		expect(() => getSlackUserAlt(slackUserId, validJSON)).to.throw(
			expected.message,
		);
	});
	...
	it("should throw an error -- No Departments", () => {
		...
		expect(() => getSlackUserAlt(slackUserId, invalidJSON)).to.throw(
			expected.message,
		);
	});
	...
	it("should throw an error -- No Users found", () => {
		...
		expect(() => getSlackUserAlt(slackUserId, invalidJSON as any)).to.throw(
			expected.message,
		);
	});
