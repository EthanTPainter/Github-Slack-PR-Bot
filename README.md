...
**GITHUB_OAUTH_TOKEN_SSM_PARAM**: *string* | An AWS SSM string that securely stores the GitHub OAuth token in the AWS parameter store. This variable references the name of the parameter created in AWS SSM Parameter store to securely retrieve the GitHub OAuth token for this application. After retrieving the token, this allows the user to make requests using the [GitHub API](https://developer.github.com/v3/) based on the token's permissions. <br> If making requests to private repositories, make sure to have a token registered under the account which owns the private repositories. | The SSM parameter can be named whatever you'd like, as long as it matches the name created in the AWS Parameter Store. The value of this parameter should be a string of hexadecimal characters `abcd.....`.
**SLACK_BOT_TOKEN_SSM_PARAM:**: *string* | An AWS SSM string that securely stores the Slack Bot token in the AWS parameter store. This variable references the name of the parameter created in AWS SSM Parameter store to securely retrieve the Slack Bot token for this application. This is required for any message request/response with Slack channels. To send alerts and messages through a third party application to Slack, you will need to create an [App](https://api.slack.com/apps) in Slack. <br> After creating an App, create a bot for you app. Creating a bot will provide you with a user token and bot token. Copy the bot token and paste it here. <br><br> **Note**: This application only expects one bot to exist per application instance. This is ideal for organizations or departments using slack. | The SSM parameter can be named whatever you'd like, as long as the value here matches the name created in the AWS Parameter store. THe value of this parameter should match the format `xoxb-1234567890-123456789012-ABCDEFGHIJKLMNOPQRSTUVWX`
...
**SQS_URL:** *string* | Provide the SQS URL to use to send and retrieve messages from this SQS. | Follows the AWS SQS URL format of `https://sqs.{AWS REGION}.amazonaws.com/{AWS Account Id}/{SQS Name}`
...
**SSM_API_VERSION**: *string* | The API Version of the AWS SSM client | This application uses version `2014-11-06` and is set by default.
...
**Require_Fresh_Approvals:** *boolean* | Whether to require fresh approvals on some or all repositories the team creates pull requests from. Which repositories are affected by this setting is applied via the 'Fresh_Approval_Repositories' setting. | Set to `true` to enable fresh approvals on some or all repositories. <br><br> Set to `false` to allow stale approvals on any and all repositories.
**Fresh_Approval_Repositories:** *string[]* | A list of repository names to enforce fresh approvals on. <br><br> If the array is empty, any pull requests created by the team will be enforced with fresh approvals. | Set to `[]` to enforce fresh approvals on all repositories, for any pull request by the team. <br><br> Set to `["repoName"]` to enforce fresh approvals only on pull requests created on the repository `repoName`. 
